...
import { DEFAULT_PUBLIC_GOODS_INITIAL } from '@policylab/shared';
...
export async function runSimulation(sessionId: string, totalIterations: number): Promise<void> {
  ...
  try {
    ...
    for (let iterNum = startIter; iterNum <= endIter; iterNum++) {
      ...
      type FiscalQuality = { infrastructureQuality: number; educationQuality: number; defenseQuality: number; welfareQuality: number };
      type FiscalSpending = { infrastructure: number; education: number; defense: number; welfare: number };
      let fiscalPublicGoodsQuality: FiscalQuality | null = null;
      let fiscalCategorySpending: FiscalSpending | null = null;
      if (economyConfig.fiscalEnabled) {
        // Phase 11 D-20/D-21: wrap fiscal tick (incl. publicGoodsEscrow credit per
        // D-11 and welfare distribution) so any treasury/escrow/agent leak appears
        // in sfcBySubsystem.fiscal. Async variant because this block awaits
        // sessionRepo.updateConfig for the escrow snapshot persistence.
        // Closure RETURNS the updated quality/spending so we can assign to outer
        // bindings via destructuring — bypasses TS's loss of narrowing across
        // async callback closures.
        const fiscalOut = await accountSubsystemAsync('fiscal', snapshotTotal, async (): Promise<{ quality: FiscalQuality; spending: FiscalSpending } | null> => {
          ...
          // Return quality + spending so outer scope can pass them to iterTelemetry
          // (TS does not narrow `let` mutated inside an async closure).
          return {
            quality: fiscalDelta.updatedPublicGoods,
            spending: fiscalDelta.categorySpending,
          };
        }, sfcBySubsystem);
        if (fiscalOut) {
          fiscalPublicGoodsQuality = fiscalOut.quality;
          fiscalCategorySpending = fiscalOut.spending;
        }
      }
      ...
      // persistence / telemetry). Applies final [3,95]/[5,95] clamps per D-03/D-08.
      {
        ...
        const pressureResult = applyStructuralPressures({
          ...
          publicGoodsQuality: fiscalPublicGoodsQuality,
          lifecycleEvents: resolution.lifecycleEvents ?? [],
        });
